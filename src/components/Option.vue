...
const Option = {
  ...
  methods: {
    renderOption() {
      const { instance, node } = this;
      const optionClass = {
        "vue-treeview__option": true,
        "vue-treeview__option--disabled": node.isDisabled,
        "vue-treeview__option--selected": instance.isSelected(node),
        "vue-treeview__option--highlight": node.isHighlighted,
        "vue-treeview__option--matched":
          instance.localSearch.active && node.isMatched,
        "vue-treeview__option--hide": !this.shouldShow
      };
    ...
    renderArrow() {
      ...
      if (node.isBranch) {
        const transitionProps = {
          props: {
            name: "vue-treeview__option-arrow--prepare",
            appear: true
          }
        };
        const arrowClass = {
          "vue-treeview__option-arrow": true,
          "vue-treeview__option-arrow--rotated": this.shouldExpand
        };

        return (
          <div
            class="vue-treeview__option-arrow-container"
      ...
      if (/*node.isLeaf && */ instance.hasBranchNodes) {
        if (!arrowPlaceholder)
          arrowPlaceholder = (
            <div class="vue-treeview__option-arrow-placeholder">&nbsp;</div>
          );
    ...
    renderLabelContainer(children) {
      return (
        <div
          class="vue-treeview__label-container"
    ...
    renderCheckboxContainer(children) {
      ...
      return <div class="vue-treeview__checkbox-container">{children}</div>;
    },
    ...
    renderCheckbox() {
      ...
      const checkboxClass = {
        "vue-treeview__checkbox": true,
        "vue-treeview__checkbox--checked": checkedState === CHECKED,
        "vue-treeview__checkbox--indeterminate": checkedState === INDETERMINATE,
        "vue-treeview__checkbox--unchecked": checkedState === UNCHECKED,
        "vue-treeview__checkbox--disabled": node.isDisabled
      };

      if (!checkMark) checkMark = <span class="vue-treeview__check-mark" />;
      if (!minusMark) minusMark = <span class="vue-treeview__minus-mark" />;
    ...
    renderLabel() {
      ...
      const count = shouldShowCount
        ...
        : NaN;
      const labelClassName = "vue-treeview__label";
      const countClassName = "vue-treeview__count";
  ...
  render() {
    ...
    const listItemClass = {
      "vue-treeview__list-item": true,
      [`vue-treeview__indent-level-${indentLevel}`]: true
    };
    const transitionProps = {
      props: {
        name: "vue-treeview__list--transition"
      }
    };
